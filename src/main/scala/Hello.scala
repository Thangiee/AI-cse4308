...
trait Graph[V, W] {
  ...
  def vertexSize: Int = adjacencyMap.size

  def edgeSize: Int = edges.size
  ...
  def path(start: V, dst: V)(implicit num: Numeric[W]): Option[List[Edge[V, W]]] = {
    @tailrec
    ...
    go(start, List(start), mutable.PriorityQueue[Edge[V, W]]() ++ adjacent(start), Map.empty).map(_.values.toList)
  }
}
...
case class Edge[V, W: Numeric](from: V, to: V, weight: W) extends Ordered[Edge[V, W]] {
  ...
  def either(v: V): Boolean = (from == v) || (to == v)
